...
if ($propays = cot_payments_getallpays('acc', 'paid'))
{
    foreach ($propays as $pay)
    {
        ...
        {
            $db->update($db_users,  array('user_acc' => (int)$rproexpire), "user_id=".(int)$userid);

            /* === Hook === */
            foreach (cot_getextplugins('payacc.done') as $pl)
            {
                include $pl;
            }
            /* ===== */
